...
in
{
  ...
  programs.zsh = {
    ...
    interactiveShellInit = ''
      ...
      watson() {
        case "''${1:-}" in
          report|log|aggregate)
            # return $pipestatus[1]: devuelve el código de watson, no el de bat
            # (que con la tubería sería siempre 0).
            command watson "$@" | bat -pp; return $pipestatus[1] ;;
          *) command watson "$@" ;;
        esac
      }
